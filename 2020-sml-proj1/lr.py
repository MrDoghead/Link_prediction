...
save_as_pkl(best_parameters,'./lr_best_parameters.pkl')
...
test_x = (test_df.iloc[:,2:] - train_x.mean()) / train_x.std()
print('test x:',test_x.head(3))
test_pred = grid_search.predict_proba(test_x)
results = []
cnt = 1
for y in test_pred:
    results.append((cnt,y[1]))
    cnt += 1

# save results
save_as_csv(results,'./results/lr_19_features_prediction.csv')
